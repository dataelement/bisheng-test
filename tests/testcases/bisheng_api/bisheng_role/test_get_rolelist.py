...
from tests.utils.http_utils import prepare_data_get

@allure.epic('API服务接口自动化测试')
@allure.story('自动化接口结果')
class TestBISHENGRoleList:
    @pytest.mark.parametrize("args", CONFDATA.rolelist_cases())
    def test_role(self,args):
        ...
        res = prepare_data_get(args, bisheng_ep)
        print(res)
